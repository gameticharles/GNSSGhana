...
namespace ReinGametiMatrixLib.Matricies
{
    ...
    public class MatrixBase<T>
    {
        ...
        public virtual T get(Int32 column, Int32 row)
        {
            if (!_dataColumnRange.Contains(column + _dataColumnRange.Start)) throw new ArgumentOutOfRangeException("column");
            if (!_dataRowRange.Contains(row + _dataRowRange.Start)) throw new ArgumentOutOfRangeException("row");
            return _dataSource[column + _dataColumnRange.Start, row + _dataRowRange.Start];
        ...
        public virtual void set(Int32 column, Int32 row, T value)
        {
            if (!_dataColumnRange.Contains(column + _dataColumnRange.Start)) throw new ArgumentOutOfRangeException("column");
            if (!_dataRowRange.Contains(row + _dataRowRange.Start)) throw new ArgumentOutOfRangeException("row");
            _dataSource[column + _dataColumnRange.Start, row + _dataRowRange.Start] = value;
        ...
        /// <returns></returns>
        public virtual MatrixBase<T> SubMatrix(Int32Range columnRange, Int32Range rowRange)
        {
        ...
        public static double[,] Matrix2Array(MatrixBase<T> matrix)
        {
            ...
            for (int i = 0; i < matrix.RowCount; i++)
            {
                ...
                {
                    matArray[i, j] = Convert.ToDouble(matrix[j, i]);
                }
            }
        ...
        {
            double[,] Mat = Matrix2Array(this);
            int N_Rows, N_Columns, i, j;

            //Find The dimensions !!
            try
            {
                N_Rows = Mat.GetUpperBound(0);
                N_Columns = Mat.GetUpperBound(1);
            }
            catch
            {
            ...
            sol += String.Format("{0} rows x {1} columns\n", N_Rows + 1, N_Columns + 1);
            ...
            for (i = 0; i <= N_Rows; i++)
            {
                for (j = 0; j <= N_Columns; j++)
                {
                    if (i == 0)
                    {
                        ...
                        for (m = 0; m <= N_Rows; m++)
                        {
                            StrElem = Mat[m, j].ToString(strFormat);
                    ...
                    StrElem = Mat[i, j].ToString(strFormat);
                    ...
                    OptiString += "  " + (Mat[i, j].ToString(strFormat));
                }
        ...
        /// <returns></returns>
        public static MatrixBase<T> SubMatrix(MatrixBase<T> view, Int32Range columnRange, Int32Range rowRange)
        {
