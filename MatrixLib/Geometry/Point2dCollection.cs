...
namespace MatrixLib.Geometry
{
    public class Point2dCollection : List<Point2d>
    {
        ...
        /// </summary>
        /// <returns></returns>
        public Matrix AsAugmentedMatrix()
        {
            Matrix matrix = new Matrix(3, this.Count);
        ...
        {
            Matrix matrix = new Matrix(2, this.Count);
            for (int j = 0; j < this.Count; j++)
            {
                matrix[0, j] = this[j].AugmentedMatrix[0, 0];
                matrix[1, j] = this[j].AugmentedMatrix[1, 0];
