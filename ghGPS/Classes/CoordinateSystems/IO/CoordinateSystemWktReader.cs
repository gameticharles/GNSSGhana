...
namespace ghGPS.Classes.CoordinateSystems.IO
{
	/// <summary>
	...
	public class CoordinateSystemWktReader
	{
		...
		private static Wgs84ConversionInfo ReadWGS84ConversionInfo(WktStreamTokenizer tokenizer)
		{
			...
			{
				tokenizer.NextToken();
				info.Rx = tokenizer.GetNumericValue();

				tokenizer.ReadToken(",");
				tokenizer.NextToken();
				info.Ry = tokenizer.GetNumericValue();

				tokenizer.ReadToken(",");
				tokenizer.NextToken();
				info.Rz = tokenizer.GetNumericValue();
                ...
                {
                    tokenizer.NextToken();
                    info.Xm = tokenizer.GetNumericValue();

                    tokenizer.ReadToken(",");
                    tokenizer.NextToken();
                    info.Ym = tokenizer.GetNumericValue();

                    tokenizer.ReadToken(",");
                    tokenizer.NextToken();
                    info.Zm = tokenizer.GetNumericValue();
                }
            }
			if (tokenizer.GetStringValue() != "]")
				tokenizer.ReadToken("]");
