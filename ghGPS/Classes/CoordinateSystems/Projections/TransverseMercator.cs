...
namespace ghGPS.Classes.CoordinateSystems.Projections
{
  ...
  -----------------------------------------------------*/
			...
			: base(parameters, inverse)
		{
			...
			central_meridian = ToRadians(par_central_meridian.Value);
			lat_origin = ToRadians(par_latitude_of_origin.Value);
		...
		{
			double lon = ToRadians(lonlat[0]);
			double lat = ToRadians(lonlat[1]);
        ...
        public override double[] MetersToDegrees(double[] p)
		{
			...
			if (Math.Abs(phi) < HALF_PI)
			{
                ...
                gscale = GridScale(es, lat, lon, ToDegrees(central_meridian), scale_factor);
                conv = Convergence(lat, lon, ToDegrees(central_meridian));

                if (p.Length < 3)
					return new double[] { ToDegrees(lon), ToDegrees(lat), 0, gscale, conv };
                else
					return new double[] { ToDegrees(lon), ToDegrees(lat), p[2], gscale, conv };
			...
			else
			{
                ...
                var lon = ToDegrees(central_meridian);
                var lat = ToDegrees(HALF_PI * sign(y));
                double zone = getLongZone(lon);
                LatZones latZones = new LatZones();
                string latZone = latZones.getLatZone(lat);
                gscale = GridScale(es, lat, lon, ToDegrees(central_meridian), scale_factor);
                ...
                if (p.Length < 3)
                    return new double[] { lon, lat, 0, gscale, conv };
                else
                    return new double[] { lon, lat, p[2], gscale, conv };
