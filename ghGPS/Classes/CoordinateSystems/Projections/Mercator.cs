...
namespace ghGPS.Classes.CoordinateSystems.Projections
{
	...
	internal class Mercator : MapProjection
	{
			...
			: base(parameters, isInverse)
		{
			...
			lon_center = ToRadians(central_meridian.Value);
			lat_origin = ToRadians(latitude_of_origin.Value);
        ...
        public override double[] DegreesToMeters(double[] lonlat)
		{
			...
			else
			{
				...
				double x = _falseEasting + this._semiMajor * k0 * (lon - lon_center);
				double y = _falseNorthing + this._semiMajor * k0 * Math.Log(Math.Tan(PI * 0.25 + lat * 0.5) * Math.Pow((1 - esinphi) / (1 + esinphi), e * 0.5));

                LatZones latZones = new LatZones();
                GNSS_Functions.GetZone = getLongZone(lonlat[0]).ToString() + " " + latZones.getLatZone(lonlat[1]);
                GNSS_Functions.GetGridScale = GridScale(e2, lonlat[1], lonlat[0], ToDegrees(lon_center), k0).ToString();
                GNSS_Functions.GetConvergence = GNSS_Functions.ConvertToDMSString(Convergence(lonlat[1], lonlat[0], ToDegrees(lon_center)), false, false);
		...
		{	
            ...
            var lon = ToDegrees(dLongitude);
            var lat = ToDegrees(dLatitude);
            double zone = getLongZone(lon);
            LatZones latZones = new LatZones();
            string latZone = latZones.getLatZone(lat);
            var gscale = GridScale(e2, lat, lon, ToDegrees(lon_center), k0);
            var conv = Convergence(lat, lon, ToDegrees(lon_center));
