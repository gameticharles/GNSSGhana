...
namespace ghGPS.Classes.CoordinateSystems.Projections
{
	...
	internal class AlbersProjection : MapProjection
	{
			...
			: base(parameters, isInverse)
		{
			...
			{
				longitude_of_center = GetParameter("central_meridian"); //Allow for alternative name
				if (longitude_of_center == null)
					throw new ArgumentException("Missing projection parameter 'longitude_of_center'");
			}
			if (latitude_of_center == null)
			{
				latitude_of_center = GetParameter("latitude_of_origin"); //Allow for alternative name
				if (latitude_of_center == null)
			...
			lon_center = ToRadians(longitude_of_center.Value);
			double lat0 = ToRadians(latitude_of_center.Value);
			double lat1 = ToRadians(standard_parallel_1.Value);
			double lat2 = ToRadians(standard_parallel_2.Value);
		...
		{
			double dLongitude = ToRadians(lonlat[0]);
			double dLatitude = ToRadians(lonlat[1]);
            ...
            GNSS_Functions.GetZone = "--";
            GNSS_Functions.GetGridScale = "--";
            GNSS_Functions.GetConvergence = "--";
        ...
        public override double[] MetersToDegrees(double[] p) 
		{
			...
			if (p.Length == 2)
				return new double[] { ToDegrees(lon), ToDegrees(lat) };
			else
				return new double[] { ToDegrees(lon), ToDegrees(lat), p[2] };
		}
