...
namespace ghGPS.Classes.CoordinateSystems.Projections
{
	...
	internal class LambertConformalConic2SP : MapProjection
	{
		...
		private double center_lon=0;      /* center longitude            */
		private double center_lat=0;      /* center latitude              */
			...
			: base(parameters, isInverse)
		{
			...
			double c_lat = ToRadians(latitude_of_origin.Value);
			double c_lon = ToRadians(central_meridian.Value);
        ...
        public override double[] MetersToDegrees(double[] p)
		{
			...
			if(p.Length==2)
				return new double[] { ToDegrees(dLongitude), ToDegrees(dLatitude) };
			else
				return new double[] { ToDegrees(dLongitude), ToDegrees(dLatitude), p[2]};
		}
