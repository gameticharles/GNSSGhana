...
namespace ghGPS.Classes.CoordinateSystems.Projections
{
	...
	internal class KrovakProjection : MapProjection
	{
			...
			: base(parameters, isInverse)
		{
			...
			_latitudeOfOrigin = ToRadians(par_latitude_of_center.Value);
			central_meridian = ToRadians(24 + (50.0/60));// par_longitude_of_center.Value);
			_azimuth = ToRadians(par_azimuth.Value);
			_pseudoStandardParallel = ToRadians(par_pseudo_standard_parallel_1.Value);
			...
			es = 1.0 - (base._semiMinor * base._semiMinor) / (base._semiMajor * base._semiMajor);
			e = Math.Sqrt(es);
			...
			_alfa   = Math.Sqrt(1 + ((es * (cosL2*cosL2)) / (1 - es))); // parameter B
			_hae    = _alfa * e / 2;
			...
			double radius = Math.Sqrt(1 - es) / (1 - (es * (sinLat * sinLat)));
		...
		{
            double lambda = ToRadians(lonlat[0]) - central_meridian;
            double phi = ToRadians(lonlat[1]);
            ...
            GNSS_Functions.GetZone = getLongZone(lonlat[0]).ToString() + " " + latZones.getLatZone(lonlat[1]);
            GNSS_Functions.GetGridScale = GridScale(es, lonlat[1], lonlat[0], ToDegrees(central_meridian), scale_factor).ToString();
            GNSS_Functions.GetConvergence = GNSS_Functions.ConvertToDMSString(Convergence(lonlat[1], lonlat[0], ToDegrees(central_meridian)), false, false);
        ...
        public override double[] MetersToDegrees(double[] p)
		{
            ...
            var lon = ToDegrees(lambda + central_meridian);
            var lat = ToDegrees(phi);
            double zone = getLongZone(lon);
            LatZones latZones = new LatZones();
            string latZone = latZones.getLatZone(lat);
            double gscale = GridScale(es, lat, lon, ToDegrees(central_meridian), scale_factor);
            double conv = Convergence(lat, lon, ToDegrees(central_meridian));

            return new double[] { ToDegrees(lambda + central_meridian), ToDegrees(phi) };
		}
