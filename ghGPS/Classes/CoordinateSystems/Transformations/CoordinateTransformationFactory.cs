...
namespace ghGPS.Classes.CoordinateSystems.Transformations
{
	...
	public class CoordinateTransformationFactory : ICoordinateTransformationFactory
	{
        ...
        /// <summary>
        /// Creates a transformation in coordinate systems.
        /// </summary>
        /// <remarks>
        /// This method will examine the coordinate systems in order to construct
        /// a transformation between them. This method may fail if no path between 
        ...
        /// the DCP (e.g. throwing an exception).</remarks>
        /// <param name="sourceCS">Source coordinate system</param>
        public ICoordinateTransformation CreateFromCoordinateSystems(ICoordinateSystem sourceCS)
        {
            return CreateFromCoordinateSystems(sourceCS, sourceCS);
        }
		...
		private static ICoordinateTransformation Proj2Proj(IProjectedCoordinateSystem source, IProjectedCoordinateSystem target)
		{
			...
			return new CoordinateTransformation(source, target, TransformType.Transformation, ct,
				String.Empty, String.Empty, -1, String.Empty, String.Empty);
		}		
        ...
        public static double[] CreateCoordinateOperation(string ClassName,List<ProjectionParameter> Parameters, double[] PT,bool toDegree = true)
        {
            
            double[] transform = new double[] { };
            switch (ClassName.ToLower(CultureInfo.InvariantCulture).Replace(' ', '_'))
            {
                case "mercator":
                case "mercator_1sp":
                case "mercator_2sp":
                    //1SP
                    transform = (toDegree)? new Mercator(Parameters).MetersToDegrees(PT) : new Mercator(Parameters).DegreesToMeters(PT);
                    break;
                case "transverse_mercator":
                    transform = (toDegree) ? new TransverseMercator(Parameters).MetersToDegrees(PT) : new TransverseMercator(Parameters).DegreesToMeters(PT);
                    break;
                case "albers":
                case "albers_conic_equal_area":
                    transform = (toDegree) ? new AlbersProjection(Parameters).MetersToDegrees(PT) : new AlbersProjection(Parameters).DegreesToMeters(PT);
                    break;
                case "krovak":
                    transform = (toDegree) ? new KrovakProjection(Parameters).MetersToDegrees(PT) : new KrovakProjection(Parameters).DegreesToMeters(PT);
                    break;
                case "lambert_conformal_conic":
                case "lambert_conformal_conic_2sp":
                case "lambert_conic_conformal_(2sp)":
                    transform = (toDegree) ? new LambertConformalConic2SP(Parameters).MetersToDegrees(PT) : new LambertConformalConic2SP(Parameters).DegreesToMeters(PT);
                    break;
                default:
