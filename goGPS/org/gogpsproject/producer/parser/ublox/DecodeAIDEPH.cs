...
namespace org.gogpsproject.producer.parser.ublox
{
	...
	public class DecodeAIDEPH
	{
		...
		private const double TWO_P_4 = 16.0; // 2^4
		private const double TWO_P_M5 = 0.03125; // 2^-5
		private const double TWO_P_M19 = 0.0000019073486328125; // 2^-19
		private const double TWO_P_M29 = 0.00000000186264514923095703125; // 2^-29
		private const double TWO_P_M31 = 0.0000000004656612873077392578125; // 2^-31
		private const double TWO_P_M33 = 1.16415321826934814453125e-10; // 2^-33
		private const double TWO_P_M43 = 1.136868377216160297393798828125e-13; // 2^-43
		private const double TWO_P_M55 = 2.7755575615628913510590791702271e-17; // 2^-55
		private const double PI = 3.141592653589793238462643383279502884197169399375105820974944592;
...
//ORIGINAL LINE: public org.gogpsproject.ephemeris.EphGps decode() throws java.io.IOException,UBXException
		public virtual EphGps decode()
		{
			...
			for (int j = 3; j >= 0; j--)
			{
				bool[] temp1 = Bits.intToBits(Bits.getUInt(bytes[j]), 8);
	...
	//		baos.write(bytes);
			...
			for (int b = 0;b < sf1d.Length;b++)
			{
				for (int j = 3; j >= 0; j--)
				{
					bool[] temp1 = Bits.intToBits(Bits.getUInt(bytes[b * 4 + j]), 8);
	...
	//		System.out.println();
			eph.Week = (int)Bits.bitsToUInt(Bits.subset(bits, 8, 10));
			eph.L2Code = (int)Bits.bitsToUInt(Bits.subset(bits, 18, 2));
			eph.SvAccur = (int)Bits.bitsToUInt(Bits.subset(bits, 20, 4));
			eph.SvHealth = (int)Bits.bitsToUInt(Bits.subset(bits, 24, 6));
			eph.Iodc = (int)Bits.bitsToUInt(Bits.concat(Bits.subset(bits, 30, 2), Bits.subset(bits, 5 * 32 + 8, 8)));
			eph.L2Flag = (int)Bits.bitsToUInt(Bits.subset(bits, 1 * 32 + 8, 1));
			eph.Tgd = Bits.bitsTwoComplement(Bits.subset(bits, 4 * 32 + 24, 8)) * TWO_P_M31;
			eph.Toc = Bits.bitsToUInt(Bits.subset(bits, 5 * 32 + 16, 16)) * TWO_P_4;
			eph.Af2 = Bits.bitsTwoComplement(Bits.subset(bits, 6 * 32 + 8, 8)) * TWO_P_M55;
			eph.Af1 = Bits.bitsTwoComplement(Bits.subset(bits, 6 * 32 + 16, 16)) * TWO_P_M43;
			eph.Af0 = Bits.bitsTwoComplement(Bits.subset(bits, 7 * 32 + 8, 22)) * TWO_P_M31;
	...
	//		System.out.println();
			...
			eph.Crs = Bits.bitsTwoComplement(Bits.subset(bits, 16, 16)) * TWO_P_M5;
			eph.DeltaN = Bits.bitsTwoComplement(Bits.subset(bits, 1 * 32 + 8, 16)) * PI * TWO_P_M43;
			eph.M0 = Bits.bitsTwoComplement(Bits.concat(Bits.subset(bits, 1 * 32 + 24, 8), Bits.subset(bits, 2 * 32 + 8, 24))) * PI * TWO_P_M31;
			eph.Cuc = Bits.bitsTwoComplement(Bits.subset(bits, 3 * 32 + 8, 16)) * TWO_P_M29;
			eph.E = Bits.bitsToULong(Bits.concat(Bits.subset(bits, 3 * 32 + 24, 8), Bits.subset(bits, 4 * 32 + 8, 24))) * TWO_P_M33;
			eph.Cus = Bits.bitsTwoComplement(Bits.subset(bits, 5 * 32 + 8, 16)) * TWO_P_M29;
			eph.RootA = Bits.bitsToULong(Bits.concat(Bits.subset(bits, 5 * 32 + 24, 8), Bits.subset(bits, 6 * 32 + 8, 24))) * TWO_P_M19;
			eph.Toe = Bits.bitsToUInt(Bits.subset(bits, 7 * 32 + 8, 16)) * TWO_P_4;
			eph.FitInt = Bits.bitsToUInt(Bits.subset(bits, 7 * 32 + 24, 1));
	...
	//		System.out.println();
			...
			eph.Cic = Bits.bitsTwoComplement(Bits.subset(bits, 8, 16)) * TWO_P_M29;
			eph.Omega0 = Bits.bitsTwoComplement(Bits.concat(Bits.subset(bits, 0 * 32 + 24, 8), Bits.subset(bits, 1 * 32 + 8, 24))) * PI * TWO_P_M31;
			eph.Cis = Bits.bitsTwoComplement(Bits.subset(bits, 2 * 32 + 8, 16)) * TWO_P_M29;
			eph.I0 = Bits.bitsTwoComplement(Bits.concat(Bits.subset(bits, 2 * 32 + 24, 8), Bits.subset(bits, 3 * 32 + 8, 24))) * PI * TWO_P_M31;
			eph.Crc = Bits.bitsTwoComplement(Bits.subset(bits, 4 * 32 + 8, 16)) * TWO_P_M5;
			eph.Omega = Bits.bitsTwoComplement(Bits.concat(Bits.subset(bits, 4 * 32 + 24, 8), Bits.subset(bits, 5 * 32 + 8, 24))) * PI * TWO_P_M31;
			eph.OmegaDot = Bits.bitsTwoComplement(Bits.subset(bits, 6 * 32 + 8, 24)) * PI * TWO_P_M43;
	...
	//		System.out.println("IDOT "+Bits.bitsTwoComplement(Bits.subset(bits, 7*32+16, 14)));//pi * 2^-43
			...
			eph.RefTime = new Time(DateTimeHelperClass.CurrentUnixTimeMillis());

			if (eph.Iodc == IODE2 && eph.Iodc == IODE3)
			{
				eph.Iode = IODE3;
				return eph;
			}
			Console.WriteLine("IODC(" + eph.Iodc + "), IODE2(" + IODE2 + "), IODE3(" + IODE3 + ") not matching for SVID " + svid);
