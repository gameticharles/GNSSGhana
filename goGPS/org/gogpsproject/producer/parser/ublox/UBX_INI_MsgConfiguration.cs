...
namespace org.gogpsproject.producer.parser.ublox
{
	...
	*/
	public class UBX_INI_MsgConfiguration : UBXMsgConfiguration
	{
	  ...
	  internal static int[] Msg
	  {
		  get
		  {
			...
			refTime = new Time(c.TimeInMillis);
			...
			// I2 week 
		//    int week = 1853;
			int week = refTime.GpsWeek;
			msg[i++] = ((int)(week & 0xFF));
			msg[i++] = ((int)((week >> 8) & 0xFF));
    
			// I4 iTOW 
		//    long iTOW = 53557000l; //ms
			long iTOW = (long) refTime.GpsTime;
	  ...
	  public UBX_INI_MsgConfiguration() : base(UBXMessageType.CLASS_AID, UBXMessageType.AID_INI, Msg)
	  {
	  }
