...
namespace org.gogpsproject.producer.parser.rtcm3
{
	...
	public class Decode1004Msg : Decode
	{
		 ...
		 */
		public virtual Observations decode(bool[] bits, int week)
		{
			...
			bool DF005 = (Bits.bitsToUInt(Bits.subset(bits, start, 1)) == 1);
			start += 1;
			...
			bool DF007 = (Bits.bitsToUInt(Bits.subset(bits, start, 1)) == 1);
			start += 1;
			...
			for (int i = 0; i < DF006 ; i++) //header.getNumberGPS()
			{
				int DF009 = (int)Bits.bitsToUInt(Bits.subset(bits, start, 6));
				start += 6;
				bool DF010 = (Bits.bitsToUInt(Bits.subset(bits, start, 1)) == 1);
				start += 1;
	...
	//	        	rtcm->obs.data[index].L[0]=DF011/lam[0]+cp1;
		...
		/// <returns> GPS Epoch Time of the beginning of the right GPS week,
		...
		/// (ie. if referenceTS report Saturday but TOW is little it must select next Sunday, not previous) </returns>
	...
	//		cal.set(Calendar.DAY_OF_MONTH, mbCal.get(Calendar.DAY_OF_MONTH));
			...
			if (cal.DayOfWeek >= DayOfWeek.Friday)
			{
				...
				if (tow < 2 * 24 * 3600 * 1000)
				{
					...
					while (cal.DayOfWeek != DayOfWeek.Sunday)
					{
						cal.AddDays(1);
					}
				}
