...
namespace org.gogpsproject.consumer
{
	...
	public class CsvProducer : PositionConsumer
	{
        ...
        private DateTime dateTXT;// = new SimpleDateFormat();
        private DateTime timeTXT;// = new SimpleDateFormat();
	...
	*/
	    public virtual void writeCoordinate(int index, RoverPosition c, StreamWriter @out)
	    {
		    try
		    {
		        ...
		        //date, time
		        string d = dateTXT.AddMilliseconds(c.RefTime.Msec).ToString("dd/MM/yyyy");
		        string t = timeTXT.AddMilliseconds(c.RefTime.Msec).ToString("HH:mm:ss");
